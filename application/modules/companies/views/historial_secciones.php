...
<head>
  ...
  <script src="<?php echo base_url();?>statics/js/libraries/form.js"></script>
   <script src="<?php echo base_url();?>statics/js/libraries/jquery.balloon.js"></script>
  ...
  <script>
  $(function() {
    $('.titulo-seccion').balloon({ position: "bottom right" });
    $('.titulo-campos').balloon({ position: "bottom right" });
    $('.title-seccion').balloon({ position: "bottom right" });
     $( ".sortable" ).sortable({
   ...
   $("#form_campo").submit(function(){
        var band = 0;

        if($("#campoNombre").val() == ''){
            $("#campoNombre").css('border', '1px solid #FF0000');
            band++;
        }
        else{
            $("#campoNombre").css('border', '1px solid #ADA9A5');
        }
...
<div id="load_proyectos" class="sortable" align="center" style=" display: block; bottom: 0px; top: 350px; height:200px; width:900px; margin-left:200px;">
  <div align="left" >
    <span title="Agregar Seccion" class="glyphicon glyphicon-list titulo-seccion" data-toggle="modal" data-target="#myModal"></span>
  </div>
    <?php if($categorias!=0):?>
       <?php foreach($categorias as $categoria):?>
            ...
            <div id="elemento-<?php echo $categoria->seccionId; ?>" class="ui-state-default">
                ...
                <div class="panel-heading">
                  <div align="left">
                    <span class="glyphicon glyphicon-edit edita_seccion title-seccion" title="editar nombre de la seccion" id="<?php echo $categoria->seccionId;?>"></span>
                  </div>
                    
                  <h4 class="panel-title" id="s-<?php echo $categoria->seccionId;?>"><?php echo $categoria->seccionNombre;?></h4>
                </div>
                <div class="panel-body">
                  
                  <div align="left" >
                    <span data-toggle="modal" data-target="#myCampo" class="glyphicon glyphicon-plus-sign add_campo titulo-campos" title="Agregar campo" id="<?php echo $categoria->seccionId;?>"></span>
                  </div>
                   
                  <!--Contenido de los campos-->
                  <?php $campos=$this->Company->get_datos_result('campos', 'campoSeccionId', $categoria->seccionId);?>
                  <table class="table">
                    <?php if($campos!=0):?>
                      <?php foreach($campos as $campo):?>
                        <tr>
                          <td>
                            <h2><small><?php echo $campo->campoNombre?></small></h2>
                          </td>
                          <td>hola dos</td>
                        </tr> 
                    <?php endforeach;?>
    ...
    <div class="modal-content">
        <?php echo form_open_multipart('companies/save_seccion', array('id'=>'form_seccion','role'=>'form')); ?>
          ...
          <div class="modal-body">
             <input type="text" class="form-control"  placeholder="Nombre de la seccion" name="save[seccionNombre]" id="seccionNombre"/>
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
        <?php echo form_open_multipart('companies/save_campo', array('id'=>'form_campo','role'=>'form')); ?>
          ...
          <div class="modal-body">
             <input type="text" class="form-control"  placeholder="Nombre de la seccion" name="campo[campoNombre]" id="campoNombre"/>
